...
WebSockets per-message deflate is supported but not enabled by default, to enable it add the `webSocketCompressionSupported=true` option to the acceptor URI.
The client must also support per-message deflate and request it via the standard WebSockets extension.
